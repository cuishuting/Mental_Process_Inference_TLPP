...
def train_epoch(model, train_dataloader, param, action_type_list, mental_type_list, optimizer):
    ...
    for id, (a_pad_batch, m_pad_batch, real_a_time_num, real_m_time_num) in enumerate(train_dataloader):
        ...
        if (id+1) % 10 == 0:
            print("cur training b_id: ", id+1, "current neg ll during training is: ", neg_ll)
    ...
    with torch.no_grad():
        for id, (a_pad_batch, m_pad_batch, real_a_time_num, real_m_time_num) in enumerate(test_dataloader):
            ...
            for t_id in range(param.batch_size):
                ...
                fig, (f1_hz, f2_srv) = plt.subplots(2, 1, figsize=(8, 10))
                ...
                """add scatter on survival function"""
                surv_r_scatter_list = []
                for g_id in range(int(param.time_horizon / param.sep_for_grids)):
                    if cur_mental_oc_gr_list[g_id]:
                        surv_r_scatter_list.append(survival_rate[g_id])
                f2_srv.scatter(org_time_dict[0][t_id].cpu(), surv_r_scatter_list, marker='o', color='red')

                f2_srv.set_title("survival rate")
                f2_srv.set_xlabel('grids')
                f2_srv.set_ylabel('survival rate')
                plt.savefig("./result_visual/result_both_v_and_pt" + str(t_id) + "_" + str(param.num_iter) + "_" + str(param.num_sample) + "_trans_encoder.png")
                plt.close()
...
"""
train_data_file_str = "./Synthetic_Data/org_train_data_dict" + "_" + str(param.sep_for_grids) + "_" + str(param.num_sample) + "_" + str(param.sep_for_data_syn) + ".npy"
...
test_data_file_str = "./Synthetic_Data/test_data_dict" + "_" + str(param.sep_for_grids) + "_" + str(num_sample_test) + "_" + str(param.sep_for_data_syn) + ".npy"
if os.path.exists(test_data_file_str):
    test_data_dict = np.load(test_data_file_str, allow_pickle=True).item()

else:
    test_data_generator = Logic_Model_Generator(param.time_tolerance, param.decay_rate, param.sep_for_data_syn, param.sep_for_data_syn)
...
num_params = sum(p.numel() for p in encoder_for_hz.parameters() if p.requires_grad)
